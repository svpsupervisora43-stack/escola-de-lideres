...
const TRILHAS_PADRAO = {
  "Mídia": {
    ...
    apoio: [
      ...
      "📱 Estudo de Tendências: Análise diária de conteúdos em alta"
    ]
  },
  "Recepcionista": {
    funcoes: [
...
// ==========================================
// 2. CARREGAMENTO DOS DADOS (LOCALSTORAGE)
// ==========================================

let funcionarios = JSON.parse(localStorage.getItem('APP_FUNCIONARIOS')) || FUNCIONARIOS_PADRAO;
let trilhasCargos = JSON.parse(localStorage.getItem('APP_CARGOS_DATA')) || TRILHAS_PADRAO;
...
function renderPerfilColaborador() {
  ...
  document.getElementById('nomeColaboradorPerfil').innerText = funcionarioLogado.nome;
  document.getElementById('cargoColaboradorPerfil').innerText = funcionarioLogado.cargo;
  document.getElementById('nivelAnimateamsPerfil').innerText = funcionarioLogado.nivelAnimateams;

  const cargo = funcionarioLogado.cargo;
  const dados = trilhasCargos[cargo] || { funcoes: [], requisitos: [], apoio: [] };

  document.getElementById('tituloCargoView').innerText = `Trilha & Requisitos — ${cargo}`;

  const ulFuncoes = document.getElementById('listaFuncoesColaborador');
  ulFuncoes.innerHTML = (dados.funcoes && dados.funcoes.length > 0)
    ? dados.funcoes.map(f => `<li class="list-group-item py-2">👤 ${f}</li>`).join('')
  ...
  ulReq.innerHTML = (dados.requisitos && dados.requisitos.length > 0)
    ? dados.requisitos.map(r => `<li class="list-group-item py-2">📌 ${r}</li>`).join('')
  ...
  ulApoio.innerHTML = (dados.apoio && dados.apoio.length > 0)
    ? dados.apoio.map(a => `<li class="list-group-item bg-transparent py-2">📖 ${a}</li>`).join('')
...
function adicionarNovoFuncionario() {
  ...
  const nivel = document.getElementById('novoNivelAnimateams').value.trim() || "Nível 1 - Iniciante";
...
function carregarFormularioCargoGestor() {
  ...
  document.getElementById('txtFuncoesGestor').value = (dados.funcoes || []).join('\n');
  document.getElementById('txtRequisitosGestor').value = (dados.requisitos || []).join('\n');
...
function salvarAlteracoesTrilha() {
  ...
  trilhasCargos[cargo] = {
    funcoes: funcoesTexto ? funcoesTexto.split('\n').filter(i => i.trim() !== '') : [],
    requisitos: reqTexto ? reqTexto.split('\n').filter(i => i.trim() !== '') : [],
    apoio: apoioTexto ? apoioTexto.split('\n').filter(i => i.trim() !== '') : []
  };
...
function restaurarPadroes() {
  if (confirm("Tem certeza de que deseja restaurar as configurações originais?")) {
